...
function AVCONNECT_action_product_create($product_id)
{
    $api = new AVCONNECT_api_ave();
    $result = $api->product->create(AVCONNECT_parseProductCreate($product_id));
    AVCONNECT_addLog([
        "type" => "AVCONNECT_action_product_create",
        "product_id" => $product_id,
        "result" => $result
    ]);
}
function AVCONNECT_action_product_update($product_id)
{
    $api = new AVCONNECT_api_ave();
    ...
    if ($id) {
        ...
        AVCONNECT_addLog([
            "type" => "AVCONNECT_action_product_update",
            "product_id" => $product_id,
            "result" => $result
        ]);
    } else {
